...
namespace ScriptHotReload
{
    ...
    {
        const string kWrapperClassFullName = "ScriptHotReload.__Methods_For_Patch_Wrapper__Gen__";
        const string kGetMethodsForPatch   = "GetMethodsForPatch";
        ...
        public static void DoHook(Assembly original, Assembly patch)
        {
            ...
            var genericInstHookPairs = GetMethodsForPatchDic(patch);
        ...
        /// <summary>
        /// 获取 patch dll 内预先生成的需要hook的方法字典
        /// </summary>
        /// <returns></returns>
        static Dictionary<MethodBase, MethodBase> GetMethodsForPatchDic(Assembly patch)
        {
            var wrapperType = patch.GetType(kWrapperClassFullName);
            var genMi = wrapperType.GetMethod(kGetMethodsForPatch);
            object ret;
            try
            {
